...
| 7 | Consolidar regra visual do editor de piloti em `@modals` | Extraído `piloti-editor-classes.ts` com funções puras para classes de botões; adicionado teste focado. | `tsc` passou; teste focado de classes passou, 1 arquivo e 2 testes; `git diff --check` passou. | Prosseguir. | Estabilizar JSDoc/playbook e preparar revisão final. |
| 8 | Estabilizar documentação dos ports e slices | Atualizados README, playbooks, ADR-001 e prompt durável para refletirem `@canvas`, `@menus`, `@modals`, `@viewer-3d` e a separação entre estado lógico e runtime visual. | `rg` não encontrou referências antigas nos documentos verificados; `tsc` passou; `git diff --check` passou. | Encerrar loops. | Revisar commits locais não enviados e levantar candidatos a ADR. |
...
- Reason: oito ciclos concluídos com commits locais separados.
- Remaining work: revisar commits locais não enviados e consolidar recomendações de ADR.
