...
> Installed by `agents-bootstrap` as `.agents/examples/feature-delivery.example.md`. Load only when
> you need calibration for using optional machine-readable artifacts beside a `work-item`. Do not
  ...
  <examples>
    <example id="1">
      ...
      <content>
        ...
        4. `task-plan.json` pode ser extraído desse plano aprovado apenas para apoiar execução
           assistida.
