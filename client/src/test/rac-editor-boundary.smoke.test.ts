...
const guardedRoots = [
  ...
  'client/src/shared',
  'client/src/infra',
  'client/src/components/rac-editor/ports',
  'client/src/components/rac-editor/lib',
  'client/src/components/rac-editor/store',
];
...
const canvasInteractionPortPath = 'client/src/components/rac-editor/@canvas/ports/CanvasInteractionPort.ts';

const concreteCanvasTypePattern = /\bCanvas(?:Group|Object)\b/;

const allowedConcreteCanvasTypeRoots = [
  'client/src/components/rac-editor/@canvas',
];
...
const orientationConsumerPaths = [
  ...
  'client/src/components/rac-editor/@canvas/lib/house-auto-stairs.ts',
...
describe('fronteira arquitetural do editor RAC', () => {
  ...
  it('nao reintroduz CanvasInteractionPort como handle amplo do canvas', () => {
    const rootPath = resolve(projectRoot, 'client/src/components/rac-editor');
    const violations = [
  ...
  it('mantem o viewer 3D dependente da projecao serializavel, nao do runtime visual', () => {
    const rootPath = resolve(projectRoot, 'client/src/components/rac-editor/@viewer-3d');
    const violations = collectSourceFiles(rootPath).flatMap((filePath) => {
      const content = readFileSync(filePath, 'utf8');
  ...
  it('mantem regras puras de contraventamento concentradas no dominio', () => {
    const sharedFilePath = resolve(projectRoot, 'client/src/shared/types/contraventamento.ts');
    const sharedContent = readFileSync(sharedFilePath, 'utf8');
    const domainFilePath = resolve(projectRoot, 'client/src/domain/house/use-cases/house-contraventamento.use-case.ts');
    ...
    const violations = [
      /export function canCreateContraventamentoForNivel\b/.test(sharedContent)
        ? 'client/src/shared/types/contraventamento.ts: regra de nivel deve ficar no dominio'
        : null,
      /export function isContraventamentoDestinationEligible\b/.test(sharedContent)
        ? 'client/src/shared/types/contraventamento.ts: regra de destino deve ficar no dominio'
        : null,
      /export function hasEligiblePilotiInContraventamentoColumn\b/.test(sharedContent)
        ? 'client/src/shared/types/contraventamento.ts: regra de coluna deve ficar no dominio'
        : null,
  ...
  it('mantem geometria visual de contraventamento fora de shared/types', () => {
    ...
    const violations = [
      /\bCONTRAVENTAMENTO_COLUMN_X\b/.test(sharedContent)
        ? 'client/src/shared/types/contraventamento.ts: coordenadas visuais de coluna fora do @canvas'
        : null,
      /\bCONTRAVENTAMENTO_ROW_Y\b/.test(sharedContent)
        ? 'client/src/shared/types/contraventamento.ts: coordenadas visuais de linha fora do @canvas'
        : null,
      /export function inferContraventamentoSide\b/.test(sharedContent)
        ? 'client/src/shared/types/contraventamento.ts: inferencia geometrica fora do @canvas'
        : null,
      /export function collectOccupiedContraventamentoSides\b/.test(sharedContent)
        ? 'client/src/shared/types/contraventamento.ts: leitura geometrica de ocupacao fora do @canvas'
        : null,
  ...
  it('mantem helpers visuais de piloti fora de shared/types', () => {
    const filePath = resolve(projectRoot, 'client/src/shared/types/piloti.ts');
    const content = readFileSync(filePath, 'utf8');

    const violations = [
      /\bgetPilotiVisualHeight\b/.test(content)
        ? 'client/src/shared/types/piloti.ts: helper visual de piloti fora do slice @canvas'
        : null,
      /\bPILOTI_BASE_HEIGHT_PX\b/.test(content)
        ? 'client/src/shared/types/piloti.ts: constante visual de piloti fora do slice @canvas'
        : null,
    ].filter(Boolean);
