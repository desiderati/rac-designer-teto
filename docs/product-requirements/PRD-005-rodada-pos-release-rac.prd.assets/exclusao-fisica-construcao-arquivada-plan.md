...
Este plano foi executado em ciclos fechados, com commits separados para planejamento, regra de
sessão/porta, UI e documentação.
...
- Remover uma casa remove o `PersistedHouseRecord`, incluindo documento RAC, canvas, vistas,
  pilotis, avaliação de terreno, materiais e metadados próprios.
...
- A exclusão física deve ser separada das ações de status, porque arquivar/desarquivar e
  inativar/reativar são reversíveis e excluir não é.
...
- A exclusão granular de casa/monitor não é exceção ao bloqueio de construção concluída ou
  arquivada.
...
- objetivo: Registrar escopo, fases, arquivos prováveis, testes e decisões aprovadas.

- resultado esperado: Asset documental em
  `docs/product-requirements/PRD-005-rodada-pos-release-rac.prd.assets/`.

- verificação: Inspeção do documento e commit isolado.

- arquivos-chave:
  - `docs/product-requirements/PRD-005-rodada-pos-release-rac.prd.assets/exclusao-fisica-construcao-arquivada-plan.md`

- commit esperado: `docs(rac): documentar plano de exclusao fisica`

### Ciclo 2: Implementar Regras Na Sessão e Nas Portas

- objetivo: Criar os comandos de exclusão física e garantir as cascatas no aggregate local.

- resultado esperado: `deleteArchivedConstructionSite(constructionSiteId)`,
  `deleteArchivedHouse(houseId)` e `deleteInactiveMonitor(monitorId)` disponíveis na sessão, bridge,
  controller e porta de gerenciamento.
...
- objetivo: Expor as ações destrutivas nas listagens permitidas e confirmar a cascata.

- resultado esperado: Construções arquivadas exibem `Excluir definitivamente`; construções não
  arquivadas não exibem a ação. Casas arquivadas e monitores inativos em construção navegável exibem
  exclusão definitiva. As confirmações chamam os comandos corretos.
...
- commit executado: `8eaf421 feat(rac): adicionar exclusao definitiva nas listagens`

- observação: O ciclo também corrigiu a propagação de `markHouseRacPrinted` pela porta padrão,
  porque a revisão identificou drift de contrato na fronteira do bootstrap.

### Ciclo 4: Atualizar Regras De Negócio

- objetivo: Consolidar as regras de exclusão física nos documentos de status.

- resultado esperado: `BUS-011` descreve que construção arquivada bloqueia edição, mas permite
  desarquivar ou excluir definitivamente a construção inteira. `BUS-010` descreve que casa arquivada
  pode ser excluída fisicamente em construção navegável.

- verificação: Inspeção documental e validação de formatação dos documentos tocados.
...
- objetivo: Executar `documentation-curator` em `docs/` e aplicar formatação correta.

- resultado esperado: Documentos em `docs/` formatados conforme governança local, preservando
  conteúdo existente.

- verificação: Rodar formatter/checker de Markdown quando disponível e revisar diff.
...
- objetivo: Confirmar que as mudanças foram endereçadas, registrar changelog e fechar work-item
  local.

- resultado esperado: Testes focados e lint executados; changelog atualizado; work-item
  reconciliado.
...
- commit esperado: Sem commit obrigatório se apenas artefatos locais `.agents/` forem alterados e
  estiverem ignorados.
