...
Descrever como hooks e estado estão organizados hoje no repositório e quais guardrails valem para
evoluções futuras. Este documento distingue a store injetada que existe hoje da coordenação lógica
da casa, que ainda não foi substituída.

## Quando criar um hook customizado

Extraia lógica para um hook customizado quando houver comportamento reutilizável, ligação com estado
compartilhado ou coordenação entre UI e dependências externas.

## Forma de retorno

Prefira retornar objeto em vez de array. Isso deixa o contrato mais explícito e reduz erro de ordem
na desestruturação.
...
- `EditorStore` concentra estado serializável de interação do editor, hoje começando por seleção
  pública e commands.

- `EditorStore` não é a fonte de verdade da casa e não substitui o controller transitório.

- `useEditorPorts` expõe ports compostos no bootstrap para leitura/escrita de casa, runtime,
  documento, projeção 3D e settings.
...
- `useHouseRuntimeSnapshot` expõe o snapshot de runtime visual quando o consumidor precisa da
  projeção do canvas.
...
- O estado estrutural da casa não deve ser duplicado em hooks locais desconectados do
  controller/ports da casa.
...
- Leituras que dependem da projeção visual devem preferir `HouseRuntimeSnapshotPort` e
  `useHouseRuntimeSnapshot`.

- Qualquer migração de responsabilidade para `EditorStore` deve remover ou substituir explicitamente
  a responsabilidade equivalente no controller/ports, evitando duas fontes de verdade.

## Papel dos hooks na feature editor

- Ler dados compartilhados do editor.

- Disparar comandos e atualizações sobre ports da casa ou abstrações equivalentes que existirem de
  fato.
...
- Não espalhe Fabric para hooks genéricos, `shared`, `domain`, `infra` ou componentes fora do slice
  `canvas`.
...
- No editor RAC, a API `window.__racDebug` é instalada por `@canvas/hooks/useCanvasDebugBridge.ts` e
  montada em `@canvas/lib/canvas-debug-bridge.ts`.

- Hooks gerais podem acionar essa ponte, mas não devem reconstruir diretamente a API global nem
  importar tipos concretos do canvas para isso.
...
- Não documente `HouseStateStore` ou substituição completa do controller da casa antes da
  implementação real.

- Toda transição de coordenação de estado deve ser explícita, incremental e acompanhada de
