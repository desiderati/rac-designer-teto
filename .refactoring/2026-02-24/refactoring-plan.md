...
// Depois: as CanvasObject (tipo definido)
...
export function readLinearObjectState(object): LinearObjectState
```
...
export interface ElementEditorStrategy {
    readState(object: CanvasObject): unknown;

    applyChanges(params: { canvas, object, ...changes }): void;
}

class LinearEditorStrategy implements ElementEditorStrategy {
    readState(object) {
        return readLinearObjectState(object);
    }

    applyChanges(params) {
        return applyLinearEditorChange(params);
    }
}
```
...
- ✅ Melhorar tipagem
- ✅ Centralizar setCanvasObjectMyType
...
- **Tipos:** `src/components/rac-editor/hooks/canvas.ts`
