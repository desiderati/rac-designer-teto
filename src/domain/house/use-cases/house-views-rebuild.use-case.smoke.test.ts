...
describe('house views rebuild use cases', () => {
  it('rebuilds normalized views with inferred type/side and unique ids', () => {
    ...
    const rebuilt = rebuildViewsFromSources({
      houseType: 'tipo3',
      sources: [
        {group: g1, metadata: {houseView: 'top', houseInstanceId: '  '}},
        {group: g2, metadata: {houseView: 'side', isRightSide: true}},
        {group: g3, metadata: {houseView: 'side', isRightSide: false}},
        {group: g4, metadata: {houseViewType: 'back', houseSide: 'top', houseInstanceId: 'dup'}},
        {group: {id: 'g5'}, metadata: {houseViewType: 'back', houseSide: 'bottom', houseInstanceId: 'dup'}},
      ],
    });
  ...
  it('ignores sources that cannot infer a valid view type', () => {
    const rebuilt = rebuildViewsFromSources({
      houseType: null,
      sources: [
        {group: {id: 'x'}, metadata: {houseView: 'unknown'}},
      ],
    });
