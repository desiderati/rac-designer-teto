...
import type {CanvasHandle, ContraventamentoCanvasSelection} from '@/components/rac-editor/Canvas';
import {refreshHouseGroupsOnCanvas, removeContraventamentosFromGroup} from '@/lib/canvas-utils';
import {houseManager} from '@/lib/house-manager';

type ProjectMetaObject = FabricObject & {
  myType?: string;
...
interface UseRacProjectActionsArgs {
...
export function useRacProjectActions({
  canvasRef,
  getCanvas,
  setInfoMessage,
  ...
  const handleDelete = useCallback(() => {
    ...
    if (selectedContraventamento) {
      const removed = removeContraventamentosFromGroup(
        ...
        (object) => (object as ProjectMetaObject).contraventamentoId === selectedContraventamento.contraventamentoId
      );
      if (removed > 0) {
    ...
    for (const object of activeObjects) {
      const typedObject = object as ProjectMetaObject;
