...
import {ActiveSelection, Canvas as FabricCanvas, FabricObject, Group} from 'fabric';
import type {CanvasHandle} from '@/components/rac-editor/Canvas';

type GroupingMetaObject = FabricObject & {
  isMacroGroup?: boolean;
  myType?: string;
};

interface UseRacGroupingActionsArgs {
...
export function useRacGroupingActions({
  canvasRef,
  getCanvas,
  setInfoMessage,
  setShowUngroupConfirm,
}: UseRacGroupingActionsArgs) {
  ...
  const handleUngroup = () => {
    ...
    if (!(group as GroupingMetaObject).isMacroGroup) {
  ...
  const handleGroup = () => {
    ...
    const typedGroup = group as GroupingMetaObject;
