...
import type {RacSubmenu} from './useRacModalState';
import type {TutorialStepId} from './useRacTutorialFlow';

interface UseRacMenuTutorialActionsArgs {
...
export function useRacMenuTutorialActions({
  tutorialStep,
  advanceTutorial,
  completeTutorial,
  isMenuOpen,
  ...
  const toggleSubmenu = useCallback((submenu: 'elements' | 'lines' | 'overflow', tutorialGate?: TutorialStepId) => {
    disableDrawingMode();
